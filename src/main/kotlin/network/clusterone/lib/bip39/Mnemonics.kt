package network.clusterone.lib.bip39

import network.clusterone.lib.bip39.Mnemonics.DEFAULT_PASSPHRASE
import network.clusterone.lib.bip39.Mnemonics.ITERATION_COUNT
import network.clusterone.lib.bip39.Mnemonics.KEY_SIZE
import network.clusterone.lib.bip39.Mnemonics.MnemonicCode
import network.clusterone.lib.bip39.Mnemonics.PBE_ALGORITHM
import network.clusterone.lib.bip39.Mnemonics.WordCount
import network.clusterone.lib.crypto.FallbackProvider
...
object Mnemonics {
    ...
    class MnemonicCode(val chars: CharArray, val languageCode: String = Locale.ENGLISH.language) :
        ...
        private val wordCount get() = chars.count { it == ' ' }.let { if (it == 0) it else it + 1 }
...
fun MnemonicCode.toSeed(
    ...
    return (DEFAULT_PASSPHRASE.toCharArray() + passphrase).toBytes().let { salt ->
        PBEKeySpec(chars, salt, ITERATION_COUNT, KEY_SIZE).let { pbeKeySpec ->
            runCatching {
